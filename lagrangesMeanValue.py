...
def getGradOfAB():
    """gradient of the line using points"""
    axx, ayy = apoint.get_xdata()[0], apoint.get_ydata()[0]
    bxx, byy = bpoint.get_xdata()[0], bpoint.get_ydata()[0]
    return (byy - ayy) / (bxx - axx)
...
def updateLinearAB():
    ...
    slope = getGradOfAB()
    cxs, cys = meanValueLoop(slope)
    mvt_points.set_data(cxs, cys)
...
initial_slope = getGradOfAB()
cxs0, cys0 = meanValueLoop(initial_slope)
mvt_points, = ax.plot(cxs0, cys0, 'go', label='MVT point(s)')
